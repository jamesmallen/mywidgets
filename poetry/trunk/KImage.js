...
function KImage()
{
  ...
  {
    for (var i in img) {
      // HACK to make sure imgs are removed from all potential superviews
      while (img[i].superview) {
        img[i].removeFromSuperview();
      }
      // HACK not really sure how to get rid of the whole object...
      // delete img[i];
      // img[i] = null;
    }
