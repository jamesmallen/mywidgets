...
function opi(i,o,w,h) {
	var t,c,r=false,f=system.widgetDataFolder+'/opi',f1=f+'.png',f2=f+'o.png';
	typeof(w)=='undefined'?w=i.srcWidth:w;
	typeof(h)=='undefined'?h=i.srcHeight:h;
	opi._cv?t=opi._cv:opi._cv=t=make(Canvas);
	c=t.getContext('2d');
...
function updprf() {
	...
	subj==''?subj=system.widgetDataFolder+'/sample':subj;
...
function ball() {
	...
	addops(shadgrad,0,0,0,0,.4,.3,.28,.7,.07,1,0);
	$.fillStyle=shadgrad;
	$.fillRect(-.25,-1,.5,.5);
	$.restore();
	...
	addops(ballgrad,0,0,0,0,0,.75,.205,0.9,.4,1,.59);
	$.fillStyle=ballgrad;
	$.fillRect(-1,-1,2,2);
	$.restore();
	...
	addops(reflgrad,255,255,255,0,.32,1,.08);
	$.fillStyle=reflgrad;
	$.fill();
	$.restore();
}
...
function rotatesubj() {
	...
	if (filesystem.isDirectory(subj)) {
		...
		for (i=0;i<glob.length;i++) {
			/\.(png|jpe?g|gif)$/i.test(glob[i])?imgs.push(subj+'/'+glob[i]):imgs;
		}
		if (imgs.length) {
...
function makesamples() {
	var c,$,i,j,k,path=system.widgetDataFolder+'/sample',atts={width:256,height:256};
	!filesystem.itemExists(path)?filesystem.createDirectory(path):path;
	// gears
	c=make(Canvas,atts);
	$=c.getContext('2d');
	$.scale(128,-128);
	...
	for (i=0;i<2;i++) {
		// fog
		c=make(Canvas,{width:64,height:64});
		$=c.getContext('2d');
		j=plasma(64,.1,1,32,false);
		k=plasma(64,10,191,32,true);
		k=chops(k,64,'+');
		rastarr($,64,k,k,k,j);
...
function plasma(sz,r,s,q,w) {
	...
	for (y=0;y<sz;y++) {
		for (x=0;x<sz;x++) {
			ret.push(Math.min(s,Math.max(0,w?Math.floor(t[y*szi+x]):t[y*szi+x])));
		}
	}
